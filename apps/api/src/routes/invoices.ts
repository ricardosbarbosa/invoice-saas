...
const invoices: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.post(
    ...
    async (request, reply) => {
      ...
      const created = await fastify.prisma.$transaction(async (tx) => {
        ...
        const currency = (body.currency || client.currency || defaultCurrency).toUpperCase()
  ...
  fastify.patch(
    ...
    async (request, reply) => {
      ...
      const hasNonStatusChanges = Object.keys(body).some((key) => key !== "status")
      ...
      const issueDate = body.issueDate ? new Date(body.issueDate) : existing.issueDate
